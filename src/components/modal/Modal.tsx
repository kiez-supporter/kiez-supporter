...
import { prettifyLink } from "../../helper/links"
    ...
    const linkModal = (
        <div className={`bg-white absolute rounded-xl left-0 top-0 h-full w-64 ${showLinks ? "block" : "hidden"}`}>
            <div className="p-5 flex flex-col">
                {links?.map((link, key) => (
                    <a
                        className="mt-2 block w-full rounded-xl mx-auto bg-logo-orange text-center p-2"
                        href={link}
                        key={key}
    ...
    return (
        ...
        >
            <div>
                ...
                <div className="mt-5">
                    <div className="w-full bg-light-grey border-black h-5 rounded-xl">
                        <div
                            className={`bg-${getColor(progress || 0)} h-5 rounded-xl text-center`}
                ...
                <div className="w-full mt-5">
                    {links ? (
                        <button
                            className="block w-full rounded-xl mx-auto bg-logo-orange text-center p-2"
                            onClick={handleClick}
                        >
                            Unterstützen
                        </button>
